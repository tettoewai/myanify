...
/** Shuffle upNext while keeping manual user-queue inserts at the front. */
...
export function restoreUserUpNext(
  ...
  for (const entry of entries) {
    ...
    if (song) {
      restored.push(createQueueItem(song, entry.source));
    }
  }
  return restored;
}

/** Pick other tracks when similar/radio has nothing left (repeats when all are seen). */
