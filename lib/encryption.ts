 ...
 */
function deriveKey(userId: string, masterSecret: string): Buffer {
    const masterKey = process.env.ENCRYPTION_MASTER_KEY || masterSecret;
...
export function encrypt(data: Buffer, userId: string): string {
    const key = deriveKey(userId, process.env.ENCRYPTION_MASTER_KEY || '');
 ...
 */
export function validateLicenseKey(licenseKey: string, userId: string, deviceId: string): boolean {
    try {
